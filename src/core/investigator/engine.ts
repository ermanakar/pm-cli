...
export async function runInvestigation(
  ...
  config: InvestigationConfig = { maxTurns: 10, maxTimeMs: 3 * 60 * 1000 }
...
6. If you learn a PERMANENT fact about the architecture (e.g. libraries used, patterns), use 'save_knowledge'.
7. Accumulate evidence.
8. When you have enough information, call 'submit_report'.
...
`;
  ...
  const messages: LLMMessage[] = [
    ...
    { role: 'user', content: `Objective: ${objective.text}` }
  ];

  let turns = 0;
  const startTime = Date.now();
  const spinner = new Spinner('Investigating...');
  ...
  try {
    while (turns < config.maxTurns) {
      ...
      // Display Thinking via Spinner update instead of log
      if (content) {
        spinner.update(`Thinking: ${content.slice(0, 60).replace(/\n/g, ' ')}...`);
      }
      ...
      // Stop spinner before tool logs to prevent glitches
      spinner.stop();
      ...
      for (const toolCall of toolCalls) {
        ...
        try {
          if (fnName === 'read_file') {
            ...
            if (!args.pattern) throw new Error('Missing "pattern" argument');
          ...
          } else if (fnName === 'save_knowledge') {
            await memoryManager.addInsight(args.insight);
            result = 'Insight saved to permanent memory.';
            logToolEvent({ type: 'shell', target: 'memory', status: 'ok', message: 'Knowledge saved.' });
          }
        } catch (err) {
  ...
  // Aggregate all findings from the conversation history
  ...
  return {
    objective,
    summary: "Investigation timed out. Partial findings below.",
    details: `The agent did not submit a final report in time. Here is the aggregated context from the investigation:\n\n## Agent Thoughts\n${thoughts}\n\n## Tool Outputs\n${toolOutputs}`,
    evidence: []
  };
}
